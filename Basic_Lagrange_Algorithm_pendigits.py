...
# -*- coding: utf-8 -*-
"""
Created on Tue Jul  9 14:03:08 2019
...
def main():
    ...
    labels = np.unique(sub_dataset[:,-1])

    # calculate weights for distance metric using one-way ANOVA for each attribute
    weights = calculate_attribute_strength(sub_dataset, labels)
    print(weights)

    # calculate distance between all points pair-wise
    distances = scipy.spatial.distance.pdist(sub_dataset[:,:-1], 'wminkowski', p = 2, w = weights)
    index_mapping = list(it.combinations(range(sub_dataset.shape[0]),2)) # mapping for values in pdist
    ...
    data_class = np.zeros((sub_dataset.shape[0], len(labels)))

    # assign vector value for each data class ie. (0,...,1,...,0)
    for i in range(sub_dataset.shape[0]):
        for j in range(len(labels)):
            if sub_dataset[i,-1] == labels[j]:
                data_class[i,j] = 1
    ...
    # compute laplacian
    degree_bounds = [3,10]
    laplacian = find_normalized_laplacian_matrix(copy(mapped_distances),  degree_bounds,sub_dataset.shape[0])

    # use each group as test group once each; use all other groups for training
    for l in range(num_iterations):

        # Split data into groups for cross validation
        split_indices = split_data_cross(copy(sub_dataset), labels, train_percent, num_groups)
        iteration_accuracy = []
        ...
        for j in range(num_groups):
            ...
            train_indices = copy(np.delete(copy(split_indices), j, 0))
            train_indices = sorted(np.asarray([i for train_indices in train_indices for i in train_indices]))

            # use nearest neighbors to predict tags for test group
            predictions = get_predictions(copy(train_indices), copy(test_indices), copy(data_class), copy(mapped_distances), degree_bounds,laplacian)

            # evaluate accuracy of model
            prediction_accuracy = get_accuracy(test_indices, copy(sub_dataset), labels, predictions)
...
def calculate_attribute_strength(data, labels):
    ...
    for i in range(data.shape[1] - 1):
        for k in range(len(labels)):
            split_data[k] = data[[j for j in range(data.shape[0]) if data[j, -1].all == labels[k].all],i]
...
def split_data_cross(data, labels, train_percent, num_groups):
    ...
    for i in range(len(labels)):
        ...
        indicies_split_class = np.array_split(indices_random,num_groups)

        # store random groups from class
        for j in range(num_groups):
            indices_split[j] += list(indicies_split_class[j])

    # sort indices
    for i in range(num_groups):
        indices_split[i] = sorted(indices_split[i])

    return np.asarray(indices_split, object)
...
def get_predictions(train_indices, test_indices, data_class, distances, degree_bounds, laplacian):
    ...
    L1 = laplacian[:,train_indices]
    L2 = laplacian[:,test_indices]
    ...
    for i in range(num_classes):
        # build matrix b
        b = -np.dot(L1, data_class[train_indices,:][:,i])
